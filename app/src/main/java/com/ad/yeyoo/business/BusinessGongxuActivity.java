...
public class BusinessGongxuActivity extends Activity {
...
//                    getLabelForGoods("0000090000000002EBFB06BC,000008000000000000000000");
                ...
                case R.id.tv_ensure:
                    if (!mTagIDArrayList.isEmpty()) {
                        ...
                        ids=ids.substring(0,ids.length()-1);
                        confirmGongxuForGoods(ids,gongxuId);
                    }
//                    confirmGongxuForGoods("0000090000000002EBFB06BC,000008000000000000000000",gongxuId);
                    break;
                case R.id.tv_cancel:
                    if (!mTagIDArrayList.isEmpty()) {
                        String ids="";
                        ...
                        ids=ids.substring(0,ids.length()-1);
                        cancelGongxuForGoods(ids,gongxuId);
                    }
//                    cancelGongxuForGoods("000008000000000000000000",gongxuId);
                    break;
            }
        }
    ...
    //-------------------base function------------
    private void initUI() {
        gongxuId=getIntent().getStringExtra("gongxuId");
        gongxuName=getIntent().getStringExtra("gongxuName");
        tv_titel=findViewById(R.id.tv_titel);
        tv_titel.setText("工序流转["+gongxuName+"]");
...
*/
    ...
    public void getLabelForGoods(String ids){
        ...
        params.put("ids",ids);
        if (BaseApplication.isNetworkAvailable(BusinessGongxuActivity.this)) {
    ...
    public void confirmGongxuForGoods(String ids,String gongxuId){
        HttpParams params=new HttpParams();
        params.put("ids",ids);
        params.put("gongxuId",gongxuId);
        if (BaseApplication.isNetworkAvailable(BusinessGongxuActivity.this)) {
            OkGo.<String>post(UrlUtils.CONFIRM_GONGXU_FOR_GOODS)     // 请求方式和请求url
                    .tag(this)                       // 请求的 tag, 主要用于取消对应的请求
                    .cacheKey("cacheKey")            // 设置当前请求的缓存key,建议每个不同功能的请求设置一个
                    .cacheMode(CacheMode.DEFAULT)    // 缓存模式，详细请看缓存介绍
                    .params(params)
                    .execute(new StringCallback() {
                        ...
                        public void onSuccess(Response<String> response) {
                            ...
                            if (checkBean.isSuccess()){
                                ...
                                Toast.makeText(BusinessGongxuActivity.this,checkBean.getMessage(),Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
        }
    ...
    public void cancelGongxuForGoods(String ids,String gongxuId){
        HttpParams params=new HttpParams();
        params.put("ids",ids);
        params.put("gongxuId",gongxuId);
        if (BaseApplication.isNetworkAvailable(BusinessGongxuActivity.this)) {
            OkGo.<String>post(UrlUtils.CANCEL_GONGXU_FOR_GOODS)     // 请求方式和请求url
                    .tag(this)                       // 请求的 tag, 主要用于取消对应的请求
                    .cacheKey("cacheKey")            // 设置当前请求的缓存key,建议每个不同功能的请求设置一个
                    .cacheMode(CacheMode.DEFAULT)    // 缓存模式，详细请看缓存介绍
                    .params(params)
